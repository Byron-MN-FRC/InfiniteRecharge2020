...
addChild("limitSwitchDown",limitSwitchDown);
    ...
    public boolean ready2Shoot(double rpms, double hoodEncoderUnits) {
        ...
        double currentRPMs = velocityPer100msToRpm(shootMotor.getSelectedSensorVelocity(BallShooterConstants.kPIDLoopIdx));
        double currentHoodPos = hoodMotor.getSelectedSensorPosition(BallShooterConstants.kPIDLoopIdx);


        boolean shootMotorAtSpeed = Math.abs(currentRPMs - rpms) < BallShooterConstants.kShootMotorRPMTolerance;
        boolean hoodAtPosition = Math.abs(currentHoodPos - hoodEncoderUnits) < BallShooterConstants.kHoodPositionTolerance;
        hoodAtPosition = true;
        ...
        //toConsole("ready2Shoot: ShootTarget=" + shootMotor.getSelectedSensorVelocity() + " Err=" + shootMotor.getClosedLoopError(BallShooterConstants.kPIDLoopIdx));
        if (++loop >= 10){
            toConsole("------------: ShootTarget=" + rpmToVelocityPer100ms(rpms) + 
            " Cur=" + shootMotor.getSelectedSensorVelocity(BallShooterConstants.kPIDLoopIdx) + 
            " Err=" + shootMotor.getClosedLoopError(BallShooterConstants.kPIDLoopIdx));
        }
        ...
        // toConsole("             HoodTarget=" + hoodEncoderUnits + " Err=" + 
            // hoodMotor.getClosedLoopError(BallShooterConstants.kPIDLoopIdx));
        toConsole("             Shooter Ready=" + shootMotorAtSpeed + " Hood Ready=" + hoodAtPosition + " settled" + isWithinThreshold());
        return isWithinThreshold() && shootMotorAtSpeed && hoodAtPosition;
    }
    ...
    private boolean isWithinThreshold() {
        ...
        if (shootMotor.getClosedLoopError(BallShooterConstants.kPIDLoopIdx) < +BallShooterConstants.kErrThreshold 
            && shootMotor.getClosedLoopError(BallShooterConstants.kPIDLoopIdx) > -BallShooterConstants.kErrThreshold) {
            ++_withinThresholdLoops;
            toConsole("incrementing threshold loops: " + _withinThresholdLoops);
        } else {
             _withinThresholdLoops = 0;
             toConsole("restart threshold loops: " + _withinThresholdLoops);
        }
